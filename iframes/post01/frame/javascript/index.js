...
window.addEventListener("load", function () {
    ...
    function updatePosts(posts_data) {
        ...
        let parabtns = document.createElement('p');
        parabtns.classList.add('older-newer');
        posts.appendChild(parabtns);

        // Handle older posts button
        if(posts_data.length > end) {
            let btnOlder = document.createElement('button');
            btnOlder.classList.add('older-posts');
            btnOlder.textContent = 'Older posts';
            parabtns.appendChild(btnOlder);

            btnOlder.addEventListener("click", function() {
                page++;
                updatePosts(posts_data);
            });
        }
        
        // Handle newer posts button
        if(start > 0){
            let btnNewer = document.createElement('button');
            btnNewer.classList.add('newer-posts');
            btnNewer.textContent = 'Newer posts';
            parabtns.appendChild(btnNewer);
